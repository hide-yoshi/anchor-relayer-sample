...
async fn sample() {
    let balance = devnet_client()
        .get_balance(&local_signer().await.pubkey())
        .await
        .unwrap();
...
async fn sample_airdrop() {
    ...
    let before_balance = client.get_balance(&signer.pubkey()).await.unwrap();
    let result = client
        .request_airdrop(&signer.pubkey(), LAMPORTS_PER_SOL)
        .await;
    match result {
        Ok(tx) => ic_cdk::print(format!("Airdrop successful: {:?}", tx).as_str()),
        Err(e) => ic_cdk::print(format!("Airdrop failed: {:?}", e).as_str()),
    }
    let after_balance = client.get_balance(&signer.pubkey()).await.unwrap();
    ic_cdk::println!(
...
async fn sample_instruction() {
    ...
    let instruction = Instruction::new_with_borsh(
        program,
        &(initialize_discriminant, "BTC".to_string()),
        vec![
    ...
    let block_hash = client.get_latest_blockhash().await.unwrap();
